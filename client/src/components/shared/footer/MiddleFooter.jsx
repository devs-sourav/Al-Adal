  ...
  const servicelist = [
    {
      ...
      details: "Enjoy fast shipping service.",
      icon: BsRocketTakeoff,
    },
    {
      title: "Money back guarantee",
      details: "Hassle-free refunds on eligible returns",
              ...
              <li>
                <ul className="flex items-center gap-x-5 mt-5 lg:mt-0">
                  {socialList.map((item, index) => {
                    let Logo = item.logo
                    return (
                      <Link to={item.link} key={index} className="flex hover:bg-danger transition-all ease-linear duration-200 hover:text-white justify-center items-center w-10 h-10 rounded-md bg-[#3C4454]">
